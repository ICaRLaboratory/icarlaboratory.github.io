...
`member-cv-interactions.mjs` checks a name-row role badge without reserved empty rows, compact CV height, disabled CV buttons below every student/alumni portrait, exclusion of advisor/opening cards, KO/EN mobile/desktop alignment and skipped keyboard focus. A temporary undergraduate fixture checks optional CV links, noopener and native Enter opening a new tab; no production CVs are enabled.
`member-card-layout-interactions.mjs` checks equal graduate/undergraduate student, alumni and opening card dimensions across live resizing and KO/EN switching, plus 16/24/32px root-font simulations and return to normal size; card contents must remain contained.
