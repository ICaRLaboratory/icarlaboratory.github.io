...
export async function runMemberCardLayoutChecks(browser, base) {
  ...
  try {
    ...
    for (const rootSize of [16, 24, 32, 16]) {
      ...
      for (const width of [1280, 768, 360, 320, 1280]) {
        ...
        for (const lang of ['en', 'ko']) {
          ...
          const cards = await page.locator('#grad > .person, #undergrad > .person, #alumni > .person').evaluateAll(nodes => nodes.map(node => {
          ...
          assert.equal(cards.length, await page.evaluate(() => GRAD_STUDENTS.length + UNDERGRAD_STUDENTS.length + ALUMNI.length + 2));
    ...
    return [{ name: 'Student, alumni and opening card dimensions across resize, language and enlarged text', pass: true }];
  } catch (error) {
    return [{ name: 'Student, alumni and opening card dimensions across resize, language and enlarged text', pass: false, error: String(error) }];
  } finally {
    await page.close();
  }
}
